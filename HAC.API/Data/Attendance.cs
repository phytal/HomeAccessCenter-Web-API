...
namespace HAC.API.Data
{
    ...
    {
        private static readonly string[] Names = { "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday" };
        ...
        public static List<List<List<Day>>> GetAttendances(CookieContainer cookies, Uri requestUri, string link)
        {
            ...
            string data = Utils.GetData(cookies, requestUri, link, ResponseType.MonthlyView, "Attendance");
            ...
            foreach (var key in monthKeys)
            {
                ...
                var response = Utils.GetDataWithBody(cookies, requestUri, link, ResponseType.MonthlyView, body, "Attendance");
            ...
            foreach (var document in documentList)
            {
                ...
                foreach (var week in weeks) //foreach course
                {
                    ...
                    foreach (var day in week.ChildNodes.WithIndex())
                    {
                        if (!int.TryParse(day.item.InnerText, out var date)) break;
                        
                        var attendances = new List<AttendanceRecord>();
                        var dayAttrs = day.item.Attributes;
